...
function nextIntro(introNumber) {
  ...
  if (currentSection) {
    ...
    if (introNumber === 10) {
      const tenImage = document.getElementById('ten-image');
      if (selectedX && selectedY) {
        tenImage.src = `img/ten-${selectedX}-${selectedY}.png`;
        tenImage.alt = `ten-${selectedX}-${selectedY}`;
        console.log(`ten-${selectedX}-${selectedY}.png 표시`);
      } else {
        tenImage.src = `img/default-image.png`; // 기본 이미지 설정
